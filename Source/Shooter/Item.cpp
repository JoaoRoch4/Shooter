...
void AItem::OnConstruction(const FTransform &Transform) {
	...
	if (MaterialInstance) {
		...
		DynamicMaterialInstance = UMaterialInstanceDynamic::Create(
			MaterialInstance, this);

		ItemMesh->SetMaterial(MaterialIndex, DynamicMaterialInstance);

	} else {

		PrintLogErr("AItem::OnConstruction(const FTransform &Transform): MaterialInstance was nullptr");
	}
...
void AItem::DefaultConstructor_Curves() {
	...
	//PulseCurve
	{
		...
		const static ConstructorHelpers::FObjectFinder<UCurveVector> M_PulseCurve(L"/Script/Engine.CurveVector'/Game/_Game/Assets/Curves/MaterialPulseCurve.MaterialPulseCurve'");

		if (M_PulseCurve.Succeeded()) {
			PulseCurve = M_PulseCurve.Object;
		} else {
			PrintLogErr("AItem::DefaultConstructor_Curves(): M_PulseCurve failed to load");
		}
	}
	...
	//InterpPulseCurve
	{
		...
		const static ConstructorHelpers::FObjectFinder<UCurveVector> M_InterpPulseCurve(L"/Script/Engine.CurveVector'/Game/_Game/Assets/Curves/MaterialInterpCurve.MaterialInterpCurve'");

		if (M_InterpPulseCurve.Succeeded()) {
			InterpPulseCurve = M_InterpPulseCurve.Object;
		} else {
			PrintLogErr("AItem::DefaultConstructor_Curves(): M_InterpPulseCurve failed to load");
		}
	}
}
...
FVector AItem::GetInterpLocation() {
	...
	switch (ItemType) {
		...
		case EItemType::EIT_Ammo:
		{
			...
			return  Character->GetInterpLocation(
...
void AItem::UpdatePulse() {
	...
	switch (ItemState) {
		...
		case EItemState::EIS_Pickup:
		{
			if (PulseCurve) {
				...
				PrintLogErr("AItem::UpdatePulse(): Case EItemState::EIS_Pickup: PulseCurve was nullptr");
			} 
			break;
		}
		case EItemState::EIS_EquipInterping:
		{
			...
			if (InterpPulseCurve) {
				...
				PrintLogErr("AItem::UpdatePulse(): Case EItemState::EIS_EquipInterping: InterpPulseCurve was nullptr");
			} 
			break;
		}
	}
	...
	if (DynamicMaterialInstance) {
		...
		DynamicMaterialInstance->SetScalarParameterValue(
			FName(TEXT("GlowAmount")),
			CurveValue.X * GlowAmount);

		DynamicMaterialInstance->SetScalarParameterValue(
			FName(TEXT("FresnelExponent")),
			CurveValue.Y * FresnelExponent);
...
void AItem::StartItemCurve(AShooterCharacter *Char) {
	...
	GetWorldTimerManager().SetTimer(ItemInterpTimer, this,
		&AItem::FinishInterping, Z_CurveTime);
...
void AItem::SetActiveStars() {
	...
	switch (ItemRarity) {
		...
		case EItemRarity::EIR_Damege:
			ActiveStars [1] = true; break;

		case EItemRarity::EIR_Common:
			for (int32 i = 1; i <= 2; i++) ActiveStars [i] = true; break;

		case EItemRarity::EIR_Uncommon:
			for (int32 i = 1; i <= 3; i++) ActiveStars [i] = true; break;

		case EItemRarity::EIR_Rare:
			for (int32 i = 1; i <= 4; i++) ActiveStars [i] = true; break;

		case EItemRarity::EIR_Legendary:
			for (int32 i = 1; i <= 5; i++) ActiveStars [i] = true; break;
	}
}
...
void AItem::ItemInterp(float DeltaTime) {
	...
	if (Character && ItemZ_Curve) {
		...
		const double GetInterpX_Value {FMath::FInterpTo(CurrentLocation.X, CameraInterpLocation.X, DeltaTime,
			InterpSpeed)};

		const double GetInterpY_Value {FMath::FInterpTo(CurrentLocation.Y, CameraInterpLocation.Y, DeltaTime,
			InterpSpeed)};
		...
		if (ItemScaleCurve) {
			...
			PrintLogErr("AItem::ItemInterp(float DeltaTime):->if(Character && ItemZ_Curve): ItemScaleCurve was nullptr");
		}
	} else {
		PrintLogErr("AItem::ItemInterp(float DeltaTime):->if(Character && ItemZ_Curve): Character or ItemZ_Curve was nullptr");
	}
}
